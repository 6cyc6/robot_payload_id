...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "the physical camera-box height from the referenced repo."
        ),
    )
    parser.add_argument(
...
def bootstrap_system_identification(system_identification_root):
    ...
    if not script_dir.is_dir():
        raise FileNotFoundError(f"Missing script directory under {root}")
...
def main():
    ...
    system_id_root, system_id_script_dir = bootstrap_system_identification(
    ...
    # The referenced repo uses relative "../robot_description" lookups in a few
    # Pinocchio helpers, so run its code from its script directory.
    os.chdir(system_id_script_dir)
